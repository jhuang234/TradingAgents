...
class GraphSetup:
    ...
    def setup_graph(
        ...
        return self.build_full_graph(selected_analysts)

    def _build_analyst_components(self, selected_analysts):
    ...
    def build_full_graph(self, selected_analysts=["market", "social", "news", "fundamentals"]):
        ...
        """
        analyst_nodes, delete_nodes, tool_nodes = self._build_analyst_components(selected_analysts)
    ...
    def build_analyst_graph(self, selected_analysts=["market", "social", "news", "fundamentals"]):
        analyst_nodes, delete_nodes, tool_nodes = self._build_analyst_components(selected_analysts)
        workflow = StateGraph(AgentState)
    ...
    def build_risk_graph(self):
        ...
        workflow.add_conditional_edges(
            "Aggressive Analyst",
            self._should_continue_risk_debate_only,
            {
                "Conservative Analyst": "Conservative Analyst",
                "END": END,
            },
    ...
    def build_portfolio_graph(self):
        portfolio_manager_node = create_portfolio_manager(
            self.deep_thinking_llm, self.portfolio_manager_memory
        )
        workflow = StateGraph(AgentState)
    ...
    def _should_continue_risk_debate_only(self, state: AgentState) -> str:
        next_step = self.conditional_logic.should_continue_risk_analysis(state)
